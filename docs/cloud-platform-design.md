...
周期额度、用量、额度流水、已发布套餐目录和购买订单。用量与额度页面只提供“当前额度周期”和“本月”
两个有界范围：上方统计由 Billing Service 对所选范围完整聚合，下面的明细固定为该范围最近 100 条，
不得再由浏览器使用明细反推汇总。当前周期的 granted、reserved、consumed 和 remaining 直接来自权威
`quota_bucket`；本月额度卡片展示本月发放、预占和消耗的变化量。每日 Token 图表按 `Asia/Shanghai`
自然周（周一开始）或自然月查询，支持向前/向后切换，当前及未来周期不能继续向未来翻页。订单详情页仅在后端明确声明 MOCK 可用时显示测试支付，
...
- 套餐版本、权益和周额度周期。
- 钱包余额、预占、用量和不可变账本。
- 用户日级用量与额度变化汇总；写入与对应 Usage/额度流水处于同一事务，供本月统计和周/月图表使用。
- 模型目录、价格版本和配置发布记录。
...
Billing 的日汇总只是一层可重建的查询模型，不替代原始事实。当前额度周期最多七天，用量直接通过
`(user_id, occurred_at)` 索引做有界聚合，额度余额读取当前桶；本月和任意单个自然周/月图表最多扫描
31 条用户日汇总。部署迁移会从既有原始记录回填汇总，后续结算和额度流水写入在同一事务中同步累加，
从而让统计范围完整、明细固定 100 条且查询成本可控。
