...
- `/login`：网页独立登录。
- `/console`：真实套餐、当前额度周期、刷新时间和完整周期用量概览；每日 Token 图表可按自然周或自然月切换并前后翻页。
- `/console/usage`：按“当前额度周期 / 本月”查看服务端完整汇总，调用明细固定展示所选范围内最近 100 条。
- `/console/ledger`：按“当前额度周期 / 本月”查看额度汇总，流水固定展示所选范围内最近 100 条不可变记录。
...
- `/admin/models`：模型创建、能力与计费配置、可选缓存创建价格、彼此独立且均可选的上游成本时段规则
  与套餐额度倍率规则，以及草稿编辑、关闭编辑、放弃草稿、发布、启停和版本历史。两套策略可以使用不同的时区、星期和
  时间段；未命中时分别使用默认成本和默认倍率，推理 Token 作为输出明细统一按输出费率计算。
...
用户用量和额度页面也不再从明细列表反推汇总：当前额度周期读取权威额度桶及完整周期聚合，本月和
周/月 Token 图表读取服务端日汇总。明细上限只影响列表长度，不影响页面上方的统计值。
