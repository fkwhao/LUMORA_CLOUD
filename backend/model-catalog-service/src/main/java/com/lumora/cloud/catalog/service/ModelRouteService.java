...
import com.lumora.cloud.catalog.persistence.entity.ModelRouteCostRuleEntity;
...
import com.lumora.cloud.catalog.persistence.mapper.ModelRouteCostRuleMapper;
import com.lumora.cloud.catalog.persistence.mapper.ModelRouteMapper;
...
public class ModelRouteService {
    ...
    public ModelRouteResponse createSecondary(ModelVersionEntity draft, ModelRouteInput input) {
        ProviderEntity provider = providerService.requireActiveForUpdate(input.providerId());
    ...
    public ModelRouteResponse update(ModelVersionEntity draft, String routeId, long expectedRevision, ModelRouteInput input) {
        ...
        ProviderEntity provider = providerService.requireActiveForUpdate(primary ? route.getProviderId() : input.providerId());
        validateProtocol(provider, draft.getSupportsWebSearch());
    ...
    public void validateForPublish(ModelVersionEntity draft) {
        ...
        for (ModelRouteEntity route : routes) {
            ...
            ProviderEntity provider = providerService.requireActiveForUpdate(route.getProviderId());
    ...
    private CatalogWebContracts.CostTimePricingPolicy adminCostPolicy(ModelRouteEntity route) {
        if (!Boolean.TRUE.equals(route.getCostTimePricingEnabled())) {
            return null;
        }
        return new CatalogWebContracts.CostTimePricingPolicy(
    ...
    private CatalogContracts.CostTimePricingPolicy resolvedCostPolicy(ModelRouteEntity route) {
        CatalogWebContracts.CostTimePricingPolicy policy = adminCostPolicy(route);
        if (policy == null) {
            return null;
        }
