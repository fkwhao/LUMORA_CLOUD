...
Catalog 的一个逻辑模型可以发布多条上游账号路由。Gateway 按优先级选择路由，同优先级按权重分流，
并通过 Redis 同时控制用户、逻辑模型、供应商账号和单路由并发；配置了 RPM/TPM 时还会执行分布式分钟
窗口准入。每条路由按 ID 动态建立 Sentinel 熔断资源。并发或速率容量已满、熔断、401/403、429、超时、
连接错误和 5xx 可以在响应开始前转移到下一路由；流式响应一旦开始则不会跨路由续接。
