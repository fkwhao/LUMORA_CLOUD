...
套餐采用 Cache Aside：Redis 缓存启用套餐的最新已发布版本列表、按版本 ID 查询的详情及该版本的模型
权限。默认 TTL 为 5 分钟，可通过 `lumora.billing.plan-cache-ttl` 配置。创建套餐或发布版本的事务成功
提交后切换缓存代次，旧代次的并发回填不会重新成为有效缓存；旧键自然过期。已发布版本保持只追加，
事务内读取的版本配置在提交成功后才回填，事务内读取的最新套餐列表不回填，避免发布未提交数据或
旧列表快照；回源复用调用方事务，不在额度事务内额外占用连接。Redis 故障或缓存损坏时回源 MySQL；失效失败时由 TTL
限制旧数据存活时间。模型可用性仍在读取套餐列表、购买校验时查询模型目录，不缓存过滤后的购买资格。
用户订阅状态、剩余额度、额度预占与结算不使用此缓存。
